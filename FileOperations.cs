...
namespace LinkWise.Modules.OneDrive
{
    public class FileOperations
    {
        ...
        internal async Task<List<FileStats>> GetMyFilesAsync()
        {
            ...
            var returnResults = new List<FileStats>();
            ...
            try
            {
                ...
                var filesResults = await spClient.Files.ExecuteAsync();
                ...
                foreach (IItem fileItem in files)
                {
                    ...
                    returnResults.Add(new FileStats
                    {
                        ItemID = fileItem.Id,
                        ItemName = fileItem.Name,
                        ItemSize = fileItem.Size,
                        ItemType = fileItem.Type,
                        ItemUrl = fileItem.WebUrl,
                    });
                }
            }
        ...
        internal async Task<List<FileStats>> GetMyFilesAsync(string selectedItemID)
        {
            var spClient = await AuthenticationHelper.EnsureSharePointClientCreatedAsync("MyFiles");
            ...
            try
            {
                var filesResults = await spClient.Files[selectedItemID].ToFolder().Children.ExecuteAsync();
